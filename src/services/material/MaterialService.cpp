...
/*----------------------------------------------------*/
template <>
const size_t ServiceImpl<MaterialService>::SID = __SERVICE_ID_MATERIAL;
...
void MaterialService::clear() {
    ...
    while (it != mTemplateMaterials.end()) {
        LOG_DEBUG("MaterialService::clearData Removing material %s",
...
MaterialPtr MaterialService::getWorldMaterialTemplate(unsigned int idx) {
    ...
    LOG_FATAL("MaterialService::getWorldMaterialTemplate: Could not find "
              "material for index %u",
              idx);
    return MaterialPtr();
}
...
void MaterialService::bootstrapFinished() {
    ...
    try {
        ResourceGroupManager::getSingleton().createResourceGroup(
            TEMPTEXTURE_RESOURCE_GROUP);
    } catch (Exception &e) {
        LOG_ERROR("Cannot create temporary texture/materials resource group "
                  "'%s'. Exception : %s",
                  TEMPTEXTURE_RESOURCE_GROUP.c_str(),
...
//-----------------------------------------------------------------------
void MaterialService::setWRTextureScale(unsigned int idx,
                                        std::pair<float, float> scale) {
    mTxtScaleMap.insert(make_pair(idx, scale));
}

//-----------------------------------------------------------------------
void MaterialService::createStandardMaterial(unsigned int idx,
                                             std::string matName,
    ...
    StringVectorPtr texnames =
        ResourceGroupManager::getSingleton().findResourceNames(
            resourceGroup, textureName + ".*");

    if (texnames->size() <= 0) {
        // no results, try the localised version
        // prev. path + /language/filename
        String locresname =
            mConfigService->getLocalisedResourcePath(textureName);

        LOG_INFO("Specified resource (%s) was not found, trying localized "
                 "version: %s",
                 textureName.c_str(), locresname.c_str());

        texnames = ResourceGroupManager::getSingleton().findResourceNames(
            resourceGroup, locresname + ".*");
    }
    ...
    for (; it != texnames->end(); it++) { // Try loading every given
        try {
            ...
            TextureManager::getSingleton().loadImage(textureName, resourceGroup,
                                                     tex, TEX_TYPE_2D, 5, 1.0f);
    ...
    if (!loaded)
        LOG_ERROR("Image %s was not found, texture will be invalid!",
                  textureName.c_str());

    // Construct a material out of this texture. We'll just clone the material
    // upstairs to enable lmap-txture combinations
    MaterialPtr shadMat =
        MaterialManager::getSingleton().create(matName, resourceGroup);
    ...
    TextureUnitState *tus =
        createAnimatedTextureState(shadPass, txtfile, resourceGroup, 5);
...
void MaterialService::createSkyHackMaterial(const Ogre::String &resourceGroup) {
    ...
    // The sky material does basically only write the Z value (no color write),
    // and should be rendered prior to any other material This is because we
    // render sky(box/sphere/dome/plane) first, and we want it to be visible
    // through the faces textured by this material
    std::string shaderName("@template249");

    if (!MaterialManager::getSingleton().resourceExists(shaderName)) {
        MaterialPtr SkyShader =
            MaterialManager::getSingleton().create(shaderName, resourceGroup);
        ...
        addWorldMaterialTemplate(SKY_TEXTURE_ID,
                                 SkyShader); // fixed at slot 249
    }
}
...
void MaterialService::createJorgeMaterial(const Ogre::String &resourceGroup) {
    ...
    if (!MaterialManager::getSingleton().resourceExists(shaderName)) {
        ...
        // jorge.png is compiled-in - see the
        // RenderService::prepareHardcodedMedia
...
Ogre::String MaterialService::getMaterialName(int mat_index) {
    std::string path = "";
    if ((mTextures[mat_index].fam != 0) &&
        ((mTextures[mat_index].fam - 1) < (int)mTxlistHeader.fam_count)) {
        path = mFamilies[mTextures[mat_index].fam - 1].name;
        path += "/";
    }

    path += mTextures[mat_index]
                .name; // TODO: Multiplatformness... is it solved by ogre?

    return path;
}

//------------------------------------------------------------------------------------
MaterialPtr MaterialService::getWRMaterialInstance(unsigned int texture,
                                                   int tag,
                                                   unsigned int flags) {
    ...
    if (isSky) {             // sky polygons have no lmapping
        ...
        tag = -1; // directly name after the original. This will cause the
    ...
    if (!shadMat) {
        LOG_ERROR("MaterialService: getWRMaterialInstance did not find "
                  "material for %u[%d] - overriding to Jorge",
                  texture, tag);
        ...
        if (!shadMat) {
            // TODO: Exception?
            LOG_FATAL("MaterialService: getWRMaterialInstance did not find "
                      "jorge for 0[%d] - this is Fatal and will probably crash",
                      tag);
        }
    }
...
//------------------------------------------------------------------------------------
MaterialService::TextureDimensions2D
MaterialService::getTextureDimensions(unsigned int texture) {
    TextureDimensions2D dimensions;
    ...
    LOG_ERROR("MaterialService::getTextureDimensions: Texture dimensions for "
              "%u not registered (returning 64x64)",
              texture);
    dimensions.first = 64;
    dimensions.second = 64;
    return dimensions;
...
//------------------------------------------------------
const std::string &MaterialServiceFactory::getName() { return mName; }
...
//------------------------------------------------------
const size_t MaterialServiceFactory::getSID() { return MaterialService::SID; }
